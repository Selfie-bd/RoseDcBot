...
{sender},**Edited message detected. I deleted it..!**
""")
    except:
        message.continue_propagation()   
...
async def channel(client, message):
    ...
    if message.sender_chat:
      ...
      if chat_id == sender:
        ...
        try:
            await message.delete()
            await app.ban_chat_member(chat_id, sender)
            ti = await message.reply_text(f"""
{sender}, Your message was deleted as it from channel(s). \n ❗️ channel are not allowed here""",
            )
...
async def porn(client, message):
    ...
    if not file_id and file_unique_id:
        return
    userid = message.from_user.id
    user_status = (await message.chat.get_member(userid)).status  
